...
static void* mca_common_ompio_buffer_alloc_seg ( void*ctx, size_t *size )
{
    ...
    if (NULL != buf && 0 == opal_accelerator.check_addr(buf, &tmp, &flags)) {
        opal_accelerator.host_register((void *)buf, realsize);
    }
...
static void mca_common_ompio_buffer_free_seg ( void *ctx, void *buf )
{
    ...
    if ( NULL != buf ) {
        if (0 == opal_accelerator.check_addr(buf, &tmp, &flags)) {
            opal_accelerator.host_unregister((void *)buf);
        }
        free ( buf );
    }
    return;
